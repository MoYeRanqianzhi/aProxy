...
例外——**受限重试路径**：`/count_tokens` 结尾的客户端路径（Claude Code compact
依赖的记数端点，大量镜像上游未实现、确定性 404）在有响应的失败达到 3 次尝试
后停止重试，把最后一次上游响应原样透传——确定性错误重试到天荒地老也不可能
成功，实测会挂死 compact 到客户端超时；网络错误仍是真正的瞬时类，不在此列。
