...
**受限重试路径**：客户端请求路径以 `/count_tokens` 结尾时（如
`/v1/messages/count_tokens`——Claude Code 的 compact 依赖它做上下文记数，而
大量镜像上游未实现该端点、确定性 404），总尝试第 1、2 次的有响应失败照常
重试，**从第 3 次尝试起任何一次有响应的失败立即原样透传并终止重试**（确定性
404 场景恰为 3 次总尝试；前两次重试零延迟，总耗时约 3×上游 RTT）。网络错误
不触发透传——它是真正的瞬时类，继续无限重试（其后首个有响应的失败仍受同一
规则约束）。理由：确定性错误重试到天荒地老也不可能成功，无限重试只会让
客户端永远等不到终态（实测 compact 挂到客户端超时报错，而直连/仅转发下 404
秒回、compact 一切正常）——快速给出真实响应才是正确行为。保活通道下上限
触发时以终态 SSE error 事件收场（骨架 200 已发出，真实状态码无法再回放）。

**不重试**（确定性失败，原样回放）：

- **受限重试路径从总尝试第 3 次起的有响应失败**（`/count_tokens` 等镜像
  普遍不支持的端点：透传该次失败响应，见上）
...
| count_tokens 拿到 404 / compact 时日志刷「受限重试路径达到尝试上限」 | 上游未实现该端点（镜像常见）：受限路径试满 3 次即透传真实 404，属预期行为，客户端自行处理 |
