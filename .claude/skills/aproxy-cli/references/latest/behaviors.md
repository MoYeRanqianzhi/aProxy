...
大量镜像上游未实现该端点、确定性 404），有响应的失败达到 **3 次尝试**（含
首轮，前两次重试零延迟，总耗时约 3×上游 RTT）后不再重试，把最后一次上游响应
**原样透传**。理由：确定性错误重试到天荒地老也不可能成功，无限重试只会让
客户端永远等不到终态（实测 compact 挂到客户端超时报错，而直连/仅转发下 404
秒回、compact 一切正常）——快速给出真实响应才是正确行为。仅封顶「上游有
响应」的失败，网络错误仍无限重试；保活通道下上限触发时以终态 SSE error 事件
收场（骨架 200 已发出，真实状态码无法再回放）。

**不重试**（确定性失败，原样回放）：

- **受限重试路径达到 3 次尝试**（`/count_tokens` 等镜像普遍不支持的端点：
  透传最后一次失败响应，见上）
